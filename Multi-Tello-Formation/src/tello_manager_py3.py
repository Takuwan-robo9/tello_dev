...
class Tello_Manager:
    def __init__(self):
        ...
        self.socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEPORT, 1)  # Linux特有のオプション
    ...
    def _receive_thread(self):
        ...
        while True:
            try:
                ...
                # Python 3ではbytesとstrは異なる型なので、bytesをデコードする必要があります
                response_str = self.response.decode('utf-8')
                
                # Python 3ではbytesに対してord()を使用できないため、インデックスでアクセスします
                response_sof_part1 = self.response[0] if len(self.response) > 0 else 0
                response_sof_part2 = self.response[1] if len(self.response) > 1 else 0
                
                if response_sof_part1 == 0x52 and response_sof_part2 == 0x65:
                    response_index = self.response[3] if len(self.response) > 3 else 0
