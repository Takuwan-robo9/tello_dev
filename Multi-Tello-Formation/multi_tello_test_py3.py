...
import sys
import time
from tello_manager_py3 import *
import queue as Queue  # Python 3では'Queue'が'queue'に変更されています
import time
import os
import binascii
...
def save_log(manager):
    ...
    if not os.path.exists('log'):
        try:
            os.makedirs('log')
        except Exception as e:  # Python 3では 'except Exception, e:' ではなく 'except Exception as e:' を使用
            pass
    ...
    for stat_list in log.values():
        ...
        for stat in stat_list:
            #stat.print_stats()
            str_val = stat.return_stats()  # 'str'は予約語なので変数名を変更
            out.write(str_val)
        out.write('\n')
